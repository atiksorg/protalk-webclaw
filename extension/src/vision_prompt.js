...
ПРАВИЛА:
1. Выведите ТОЧНО ОДИН JSON объект — следующее действие для выполнения (или action_chain с массивом действий).
...
12. ЭФФЕКТИВНОСТЬ: Если видите 2+ полей формы или 2+ элемента для клика — ОБЯЗАТЕЛЬНО объединяйте в action_chain. Каждая цепочка экономит один раунд скриншот→модель, что ускоряет работу в 2-3 раза и экономит токены. Одиночные действия — только когда нужен промежуточный скриншот для принятия решения.
...
ЦЕПОЧКИ ДЕЙСТВИЙ (ACTION CHAINS) — ОБЯЗАТЕЛЬНО ДЛЯ ЭФФЕКТИВНОСТИ:
Всегда используйте action_chain когда выполняете 2+ последовательных предсказуемых действия. Каждая цепочка экономит целый раунд «скриншот → модель», что ускоряет работу в 2–3 раза и экономит токены.

ОБЯЗАТЕЛЬНО ИСПОЛЬЗУЙТЕ ЦЕПОЧКИ В ЭТИХ СЛУЧАЯХ:
- Заполнение формы: type_at для каждого поля + click_at на кнопку "Отправить"/"Регистрация"
- Закрытие нескольких баннеров/уведомлений: несколько click_at подряд
- Ввод данных в несколько полей: Имя → Фамилия → Email → Телефон → Submit
- Серия прокруток для поиска элемента
- Навигация внутри одной страницы: scroll → click → wait
- Несколько однотипных действий (выбрать несколько чекбоксов, закрыть несколько попапов)
...
{"action_chain":true,"think":"Заполняю форму регистрации: имя, email, кнопка submit","actions":[
  {"tool":"type_at","x":300,"y":200,"text":"Иван"},
  {"tool":"type_at","x":300,"y":280,"text":"ivan@example.com"},
  {"tool":"click_at","x":500,"y":450}
]}

Пример — закрытие баннеров и клик:
{"action_chain":true,"think":"Закрываю cookie-баннер и баннер подписки, затем кликаю на кнопку","actions":[
  {"tool":"click_at","x":950,"y":50},
  {"tool":"wait","seconds":1},
  {"tool":"click_at","x":950,"y":50},
  {"tool":"click_at","x":500,"y":300}
]}

НЕ используйте цепочки ТОЛЬКО когда:
- Клик с высокой вероятностью откроет новую страницу (ссылки, target="_blank")
- Вы НЕ УВЕРЕНЫ в координатах элементов (нужен скриншот между действиями)
...
export function buildVisionPrompt({ task, userContext, currentUrl, pageTitle, history, step, consecutiveSame, taskMemoryContext, overlayHint }) {
  ...
  // Action chain efficiency hint: if recent steps have no chains, nudge the model
  if (history && history.length >= 3) {
    const recentChains = history.slice(-5).filter(h => {
      const actionStr = typeof h.action === 'string' ? h.action : JSON.stringify(h.action || '');
      return actionStr.includes('"action_chain"') || actionStr.includes('"_isChain"');
    }).length;
    if (recentChains === 0) {
      parts.push('💡 Подсказка: Вы выполняете действия по одному. Если на странице есть 2+ полей формы или 2+ элемента для клика — объединяйте в action_chain для ускорения работы.');
    }
  }
